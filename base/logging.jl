        ...
        if std_level >= _min_enabled_level[]
            ...
            if std_level >= logstate.min_enabled_level
                ...
                if shouldlog(logger, level, module_, file, line, id, $max_log, $progress)
                    ...
                    create_msg = function cm(logger, level, module_, filepath, line, id)
                        msg = $(esc(message))
...
"""
    SimpleLogger(min_level=Info)

Simplistic logger for logging to STDERR.
"""
struct SimpleLogger <: AbstractLogger
    min_level::LogLevel
end
SimpleLogger() = SimpleLogger(Info)

function configure_logging(logger::SimpleLogger; min_level=Info)
    SimpleLogger(parse_level(min_level))
end
...
function logmsg(logger::SimpleLogger, level, msg, module_, filepath, line, id;
    ...
    println(STDERR, "$level [$(basename(String(filepath))):$line]: $msg")
end
...
_global_logstate = LogState(BelowMinLevel, SimpleLogger())
