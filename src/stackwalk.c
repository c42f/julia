...
JL_DLLEXPORT void jlbacktrace(void)
{
    ...
    jl_static_show_backtrace(JL_STDERR,  ...)
}

JL_DLLEXPORT void jl_static_show_backtrace(uv_stream_t *out, uintptr_t* bt_data, size_t bt_size)
{
    for (size_t i = 0; i < bt_size; ) {
        if (bt_data[i] == JL_BT_INTERP_FRAME) {
            jl_safe_printf("Interpreter frame (ip: %d)\n", (int)bt_data[i+2]);
            jl_static_show(out, (jl_value_t*)bt_data[i+1]);
            i += 3;
        } else {
