...
temp_pkg_dir() do
    ...
    begin
        ...
        msg = read(`$(Base.julia_cmd()) --startup-file=no -e 'redirect_stderr(STDOUT); using Logging; current_logger!(:global,SimpleLogger(STDOUT)); Pkg.build("BuildFail")'`, String)
    ...
    let package = "Example"
        ...
        msg = read(ignorestatus(`$(Base.julia_cmd()) --startup-file=no -e "redirect_stderr(STDOUT); using Logging; current_logger!(:global,SimpleLogger(STDOUT)); Pkg.build(\"$package\")"`), String)
    ...
    let package = "Example"
        ...
        msg = read(ignorestatus(`$(Base.julia_cmd()) --startup-file=no -e
            "redirect_stderr(STDOUT); using Logging; current_logger!(:global,SimpleLogger(STDOUT)); using Example; Pkg.update(\"$package\")"`), String)
    ...
    let package = "StartupFile"
        ...
        withenv((Sys.iswindows() ? "USERPROFILE" : "HOME") => home) do
            code = "redirect_stderr(STDOUT); using Logging; current_logger!(:global,SimpleLogger(STDOUT)); Pkg.build(\"$package\")"
            ...
            code = "redirect_stderr(STDOUT); using Logging; current_logger!(:global,SimpleLogger(STDOUT)); Pkg.test(\"$package\")"
